...
public final class RecordAttendancePanel {
    ...
    public static JPanel build(List<EmployeeData> employees, List<AttendanceData> attendanceRecords) {
        ...
        recordButton.addActionListener(e -> {
            ...
            if (!clockIn.matches("\\d{1,2}:\\d{2}") || !clockOut.matches("\\d{1,2}:\\d{2}")) {
                resultLabel.setForeground(Color.RED);
            ...
            try {
                AttendanceData record = AttendanceOperations.buildRecord(empId, date, clockIn, clockOut);
                ...
                resultLabel.setText(String.format("Recorded %.2f hrs (%.2f regular, %.2f OT) for %s %s on %s.",
                        record.totalHours, record.hoursRegular, record.hoursOvertime,
                        emp.firstName, emp.lastName, date));
                idField.setText(""); dateField.setText(""); clockInField.setText(""); clockOutField.setText("");
            } catch (Exception ex) {
                resultLabel.setForeground(Color.RED);
