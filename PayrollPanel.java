...
import java.util.List;
 ...
 //  Never instantiated. build() wires the "Payroll (Semi-Monthly)" tab
 // directly to PayrollCalculator.computeSemiMonthlyPayrollFromAttendance()
 // and PayrollReportFormatter.format() — both static functions.
...
public final class PayrollPanel {
    ...
    public static JPanel build(List<EmployeeData> employees,
        ...
        computeButton.addActionListener(e -> {
            ...
            List<AttendanceData> records = AttendanceOperations.getRecordsForDayRange(
                    attendanceRecords, empId, month, year, startDay, endDay);
            if (records.isEmpty()) {
                resultArea.setText("No attendance records for "
                        + buildCoverageLabel(coverageIndex, month, year) + ".");
                return;
            }
            ...
            AuditLogger.log(user, role, "PAYROLL COMPUTED",
                    String.format("Semi-monthly payroll computed for %s %s, %s.",
    ...
    private static String buildCoverageLabel(int coverageIndex,
            int month, int year) {
